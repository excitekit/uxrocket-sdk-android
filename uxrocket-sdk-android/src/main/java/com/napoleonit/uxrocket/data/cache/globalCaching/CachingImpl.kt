...
class CachingImpl(
    ...
    companion object {
        ...
        const val LOG_IDENTITY_TASK_LIST = "LogIdentityTaskList"
    ...
    override val logIdentityTaskList: List<LogIdentityModel>
        @Synchronized get() {
            val dataJson = sharedPreferences.getString(LOG_IDENTITY_TASK_LIST, "")
            return if (dataJson.isNullOrEmpty()) {
                emptyList()
            } else {
                json.decodeFromString(ListSerializer(LogIdentityModel.serializer()), dataJson)
            }
        }

    override fun addLogIdentityTaskToQueue(logIdentityModel: LogIdentityModel) {
        val modifiedList = ArrayList(logIdentityTaskList)
        modifiedList.add(logIdentityModel)
        val dataJson =
            json.encodeToString(ListSerializer(LogIdentityModel.serializer()), modifiedList)
        sharedPreferences.edit().putString(LOG_IDENTITY_TASK_LIST, dataJson).apply()
    }

    override fun removeLogIdentityTaskList() {
        sharedPreferences.edit().remove(LOG_IDENTITY_TASK_LIST).apply()
    }
    ...
    override fun addLogCampaignEventTaskToQueue(logCampaignModel: LogCampaignModel) {
        ...
        val dataJson =
            json.encodeToString(ListSerializer(LogCampaignModel.serializer()), modifiedList)
        sharedPreferences.edit().putString(LOG_EVENT_TASK_LIST, dataJson).apply()

        "Campaign log cached.".logInfo()
    }

    override fun cacheElements(
        needCacheElements: List<ElementModel>,
    ...
    private fun addElements(activityOrFragmentName: String, elements: List<ElementModel>) {
        ...
        if (modifiedList.isElementExist(activityOrFragmentName)) {
            modifiedList[modifiedList.getElementIndex(activityOrFragmentName)] =
                ParentElementModel(activityOrFragmentName, elements)
        } else {
            modifiedList.add(ParentElementModel(activityOrFragmentName, elements))
        }

        val dataJson =
            json.encodeToString(ListSerializer(ParentElementModel.serializer()), modifiedList)
